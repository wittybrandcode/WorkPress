...
|---|---|:---:|:---:|---|---|:---:|
| **1** | [`assets/src/pages/SettingsPage.js`](file:///c:/laragon/www/WORKPRESS/wp-content/plugins/WorkPress/assets/src/pages/SettingsPage.js) | **78.8 KB** | **1,762** | يدمج 8 تبويبات إعدادات ضخمة (الهوية، الصلاحيات RBAC، قنوات الإشعارات، الوحدات، التشخيص) في ملف واحد. | `assets/src/components/settings/`<br>• `GeneralBrandingTab.js`<br>• `RolesPermissionsTab.js`<br>• `ModulesToggleTab.js`<br>• `NotificationsConfigTab.js`<br>• `DiagnosticsHealthTab.js`<br>• `SettingsPage.js` (< 150 سطر) | 🔴 **عالية جداً (P1)** |
| **2** | [`assets/src/components/GanttChart.js`](file:///c:/laragon/www/WORKPRESS/wp-content/plugins/WorkPress/assets/src/components/GanttChart.js) | **47.2 KB** | **1,236** | يخلط بين حسابات الرياضيات الزمنية، رسم متجهات SVG للروابط، وأشرطة المهام، وشريط المقاييس. | `assets/src/components/gantt/`<br>• `GanttScaleBar.js`<br>• `GanttGridCanvas.js`<br>• `GanttDependencyLinks.js`<br>• `GanttTaskRow.js`<br>• `GanttChart.js` (Lean coordinator) | 🔴 **عالية (P1)** |
