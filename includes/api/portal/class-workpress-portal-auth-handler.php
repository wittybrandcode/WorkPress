...
class WorkPress_Portal_Auth_Handler {
	...
	public function check_logged_in_permission() {
		if ( ! is_user_logged_in() ) {
			return new WP_Error( 'rest_not_logged_in', __( 'You must log in to access the projects portal.', 'workpress' ), array( 'status' => 401 ) );
		}
		return true;
	}
	...
	public function check_project_access_permission( $request ) {
		...
		if ( ! WorkPress_Portal_Service::can_user_access_project( $project_id, $user_id ) ) {
			return new WP_Error( 'rest_forbidden_project', __( 'You do not have permission to view this project.', 'workpress' ), array( 'status' => 403 ) );
		}
	...
	public function portal_login( $request ) {
		...
		if ( empty( $username ) || empty( $password ) ) {
			return new WP_Error( 'missing_credentials', __( 'Please enter username and password.', 'workpress' ), array( 'status' => 400 ) );
		}
		...
		if ( is_wp_error( $user ) ) {
			return new WP_Error( 'invalid_credentials', __( 'Invalid login credentials. Please verify and try again.', 'workpress' ), array( 'status' => 401 ) );
		}
